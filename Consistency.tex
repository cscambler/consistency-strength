...
To accommodate this, first we redefine the formulae of $\mathcal{L}_1$ so 
...
in $\mathsf{M}$ is the same between the two languages. On the one hand if somethin is a theorem in the
...
\[(y_{2n} \in y_{2k})^\Diamond:= x_n \in x_k \]
\[ y_{2n} \in y_{2k+1}^\Diamond := X_k x_n \]
...
Check the base. If $\varphi$ is $y_k \in y_n$, what happens?

Well, if $k = 2j$ and $n = 2i$, the diamond takes it 
