...
Now, any real in $L[r]$ of course features in $L[r]_{\omega_1}$.
So suppose $x = \mathbb{R}^{L[r]}$ according to $\du$ but not $\Diamond$. Then we must 
be able to find $y$ such that $(y \in L[r]_{\omega_1^{L[r]}})^\Diamond$ but not $(y \in L[r]_{\omega_1}^{L[r]})^{\du}$. 
But this, with \eqref{butt}, yields a contradiction.
We may thus infer \eqref{dtrans} from \eqref{dutrans}, since in fact the very same witness 
will suffice.
