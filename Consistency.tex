...
\begin{proof}
    ...
    For the quantifiers: $t(\forall x \psi)(T,r)^\Diamond$ being 
    ...
    $\forall x \in T \psi$. But can we necessitate?

    I'm really not sure about the truth of this. Take `every $x$ is not equal to $T$'.
    The translation of this is `necessarily, for every $x$ in $T$, $x$ is not equal to $T$.'
    That's true. OK. I guess the problems will come with contingent conditions. What about
    $\psi$ being for example `there are no infinite sets', so we have vacuous quantification.
    `everything is such that there are no infinite sets' may be true. 
    But `necessarily, for every $x \in T$, there are no infinite sets' reduces 
    to `necessarily, there are no infinite sets' which is false. NO IT DOESN'T! 
    It becomes `necessarily, there are no infinite sets IN $T$.' The point is 
    it becomes $\psi^T$ in the translation right? Then the $\Box$ drops away... 
    but make this precise.
