...
acts of set production, $\varphi$ can be made true'; that is, by repeatedly taking some things 
...
push things further. The axioms 
...
\end{equation}
assert that it is possible, by introducing enough sets, to eventually produce all possible natural
numbers (in the case of \eqref{compn}), and to eventually produce all the subsets of any given set $y$
that one can get \emph{without forcing/enumeration} (in the case of \eqref{comps}). \eqref{comps} is 
weaker than \eqref{compn} in terms of its structure since $\bu$ is weaker than $\Box$, and the reason 
for this should be clear on the intuitive readings of the modal operators: while we cannot add 
natural numbers by forcing, we can in general add subsets of infinite sets in this way, so \eqref{comps}
is the strongest we can consistently hope for in this setting. (Can we derive the stronger thing 
from the weaker one??)
