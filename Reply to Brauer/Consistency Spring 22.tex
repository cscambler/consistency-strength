...
\begin{description}
    ...
    \item[P-ext] $\forall x [Xx \equiv Yx] \supset X = Y$
...
\begin{proof}
    ...
    \begin{equation} 
        \Diamond (Ex \wedge \Psi(x, y, z) \wedge \exists X \forall w[Xw \equiv \Diamond w \in z])
    \end{equation}
    by HP, 
    \begin{equation} 
        \Diamond (Ex \wedge \Psi(x, y, z) \wedge \circledbar \exists u \forall w[w \in u \equiv \Diamond w \in z])
    \end{equation}
    one can then instantiate on $u$ and prove $\circledbar Eu \wedge u = z$ 
    in the relevant context.
    But this is a contradiction.

    why doesn't this work without $y \subseteq x$? 

    Need to show $\Diamond \circledbar \forall a[ a \in u \equiv a \in z]$. 
    Suppose $\Diamond \circledbar Ea$ and $a \in u$. Then 
    $\Diamond \circledbar Ea \wedge \Diamond a \in z$.
    By rigidity for membership, $\Diamond \circledbar Ea \wedge a \in z$.
    Conversely, suppose that $\Diamond \circledbar Ea$ and $a \in z$.
