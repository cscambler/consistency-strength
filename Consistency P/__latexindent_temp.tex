...
\begin{proof}
    ...
    \end{equation}
    abbreviate this as $\Diamond \exists x \Psi(x, y, z)$. By 
    strong comprehension and existential instantiation,
    \begin{equation} 
...
Roberts' claim is that the open instances are mandatory. But I see no good reason 
...
For another, if one looks at the role played by such instances in the argument,
one will find reasonable grounds for the potentialist to be doubtful of the instances
at an intuitive level, at least given a broadly Stalnakerian approach to the 
metaphsycis of modality. Roughly speaking, in intuitive terms, the argument 
for the inconsistency goes like this.

\begin{quote}
    Strong HWP says there could be an $x$ such that: 
    \begin{enumerate}
        \item $x$ could have $z$ as a subset in the absolute sense, but
        \item $z$ could not be attained by vertical expansion.
    \end{enumerate}
    But this is inconsistent. Suppose $x$ 
    exists at world $w$. By plural comprehension there 
    always such things as $z$s elements at $w$, and so by vertical expansion 
    those things form a set. But that set is just $z$. So it was 
    attainable by vertical expansion after all. 
\end{quote}
Should the HW-potentialist accept the instance of plural comprehension appealed 
to there? That is, at $w$, should we believe there are such things as $z$s elements?

There's a certain line of thought here (with roots in Stalnaker) that says 
clearly not. Suppose you are an inhabitant of the world $w$. In $w$, as it 
were, there is no such thing as $z$ to refer to. It is possible, in $w$, 
that there be some subset of $x$ that cannot be attained by vertical expansion; 
that much we are sure of. But we are incapable of singling out any particular such 
$z$ on which to comprehend, because there simply are no such $z$. There is 
simply the generic possibility of there being something of the relevant type, 
without it being true of any particular thing that it is possibly a thing 
of the type. 

This is a perfectly coherent position and a reasonable rational for, in the formal 
theory, restricting the allowable instances of comprehension to be closed. It amounts 
to only allowing parameters that are existent things, to which de re reference 
is possible, into the instances of comprehension. In other cases mere de dicto, 
generic reference to possibilities is all we can hope for. We can't single out 
particular witnesses to possible existentials on which to comprehend. 

All told then I do not think this argument shows HWP is inconsistent, although it 
does serve to bring out the contours of the philosophical commitments that go along 
with the view.
...
over a given domain one can find new ones by forcing. If you are really unhappy 
with this being true for `pluralities', I'm happy to drop the term: so long 
as we have something like second order quantification and th
...
almost nothing would count as an inconsistency argument here for the combination of
height and width potentialism. Any derived inconsistency would simply show 
the formalization was incorrect. (Compare Russell's paradox and the concept of set.)
