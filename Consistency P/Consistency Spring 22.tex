...
\begin{theorem}\label{SHWP1}
    ...
    theoremhood from the first order fragment of SOA to SHWP.
\end{theorem}
\begin{theorem}\label{SHWP2}
    ...
    SOA.
\end{theorem}
Theorem \ref{SHWP1} says that SHWP interprets SOA on first order formulas; i.e.,
it interprets PA. 
...
more liberal option would be to answer in the affirmative in all cases. 

However this question is answered by the potentialist, the actualist will introduce a corresponding 
formula, $World(x)$, which picks out those sets that the potentialist thinks of as possible worlds, 
as a way the sets could be. They then use that definition, along with quantification over such worlds,
to emulate the potentialists claims of possibility and necessity. 
...
set theory (although by no means everywhere in foundational research!)
...
SOA by topological regularity, rather than ZFC by large cardinals. (But you still 
get the latter in inner models!)
But the details here are for another time. 
