...
which (using Proposition \ref{SHWP0}) is not difficult. The translation fails for 
the full second order language because the translation of comprehension in these terms is 
incompatible with HP, as the reader can readily check.
...
\begin{theorem}\label{HWP0}
    ...
    that preserves theoremhood from (first order) ZFC to HWP.
\end{theorem}
\begin{theorem}\label{HWP1}
    There is a translation $\cdot^\Diamond : \mathcal{L}_\in \to \mathcal{L}_1$
    that preserves theoremhood from (first order) SOA to HWP.
\end{theorem}
