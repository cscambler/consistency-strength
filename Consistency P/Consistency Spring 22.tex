...
\end{lemma}
\begin{proof}
    By mirroring and Lemma REF, our assumption implies that there are $\Delta_0$ formulas $\psi, \theta$
    for which
...
\begin{theorem} 
    ...
    \begin{definition}[$\varphi_\exists$]
        ...
        In each case, $d_1, d_2$ are the least variables not occuring in $\varphi$ $\rho$ is the 
        set-theoretic rank function; and $\leq$ is the relation of relative constructibility for 
        reals.
        \begin{itemize}
            ...
            \item $(\circledbslash \varphi)_\exists := (\Diamond \varphi_\exists)$
        \end{itemize}
...
HWP (in the sense of the modality $\Diamond$). Since HWP proves all sets are countable (with respect the $\Diamond$ translation)
