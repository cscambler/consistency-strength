...
\title{{\bf On the Consistency Strength of Axioms for Height and Width Potentialism}}
...
Such systems are known. The simplest is SOA + $\Pi_1^1$-PSP. An important 
fact about this system is that it proves $L[r]$ is a model of ZFC for 
every real $r$. This itself is a consequence of a perhaps more fundamental 
result, that given $\Pi_1^1$-PSP, only countably many reals are constructible 
from $r$. 

This gives us the tools we need to prove the three key results for $T := SOA + \Pi_1^1-PSP$.
Describe the proofs.

\section{Conclusion}
\section*{Appendix A: The Theory $\mathsf{L}$}
\section*{Appendix B: Proofs for the key results on $\mathsf{L}$}
\section*{Appendix C: The Theory $\mathsf{M}$}
\section*{Appendix D: Proofs for the key results on $\mathsf{M}$}
\section{Some stuff}
Modal Set Theory:
